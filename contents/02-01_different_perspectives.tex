...
\begin{exam}
  \begin{exlist}
    ...
    integers. The set \(U_K\) of units in \(\algint\) and its complement are
    Diophantine over \(\algint\).

    We use that an integer \(α ∈ \algint\) is a unit if and only if
    \({N_{K/ℚ}(α)}^2 = 1\). To see that \(U_K\) is Diophantine, note that
    \(N_{K/ℚ}(X)\) is the determinant of a matrix with coefficients over \(ℚ\)
    and thus a polynomial over \(ℚ\). Since we are only asking whether \(p(α) :=
    N_{K/ℚ}(α)^2 - 1 = 0\) we can multiply \(p\) with some rational integer to
    obtain a polynomial over \(ℤ\) with identical roots. We conclude that
    \(U_K\) is Diophantine.

    Using the same polynomial we find that \(\algint\setminus U_K\) is
    Diophantine by the previous example.
