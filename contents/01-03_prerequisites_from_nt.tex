...
\begin{defin}
  A set of distinct real numbers \(\seq[n]{α} ∈ ℝ\) is called \emph{linearly
...
\begin{thm}[Multidimensional Kronecker approximation theorem]%
  \label{thm:Kronecker}
  Let \(\seq[ℓ]{α} ∈ ℝ\) be distinct real numbers that are linearly independent
...
\begin{proof}
  ...
  \[
    0 < |α_i q - g_i| < δ
  \]
  holds for all \(i ∈ \set{1, …, ℓ}\). The left inequality holds again since
  \(α_i\) is irrational.
  ...
  \begin{equation}\label{eq:Kronecker 3}
    ...
    |n| ≥ N
  \end{equation}
  hold. Furthermore, we denote \(ϑ := k - (β_ℓ + c_ℓ)/(α_ℓ q - g_ℓ)\).

  For \(1 ≤ j < ℓ\) we consider
