...
Chap.~2 of the textbook~\cite{Hlawka1986}.
...
basis, whereas its volume \(μ(D)\) is an invariant of the lattice. This is
...
\cref{fideterminate. All elements of \(Λ\) appear at intersection of the lines.
...
\begin{thm}[Minkowski's theorem on convex bodies]\label{thm:Minkowski}
  ...
  If \(T \subseteq V\) is compact, convex and symmetric in the origin, i.e.\ if
  \(α ∈ T\) so is \(-α ∈ T\), and
  \[
    μ(T) > 2^n μ(D).
  \]
...
\begin{proof}
  ...
  volume \(μ(D)\) of a fundamental parallelepiped \(D\) of \(Λ\) is \(1 \cdot
  p^2\).
...
the subject can as well be found in Chap.~23 of the textbook~\cite{Hardy1975}.
