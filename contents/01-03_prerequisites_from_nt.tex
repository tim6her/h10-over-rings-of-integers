...
ℝ\) can be made arbitrarily close to an integer by varing \(n ∈ ℕ\). The closely
related approximation theorem by Leopold Kronecker tells us, that we cannot only
approximate integral values, but if \(α\) is irrational then \(α n - β\) can be
made arbitrarily close to an integer for all \(β ∈ ℝ\).
...
\begin{proof}
  ...
  one. Secondly, we want that \(|n| ≥ N\), but since \(|n| = q |k|\), it
  suffices make sure that \(|k| ≥ N\). For this it suffices to have
  \[
    |k| > \left\vert \frac{β + c}{α q - g}\right\vert ≥
          \frac{|c| - |β|}{|α q - g|} ≥ N + 1,
  \]
  which holds if
  \[
    |c| ≥ (N + 1) |αq - g| + |β|
  \]
  is satisfied. Choose such an integer \(c\) and set \(k\) accordingly then
  \(|n| ≥ N\) holds, but we can also deduce from \eqref{eq:Kronecker 1} that
  \[
    |α n - β - p | =
      \underbrace{|αq - g|}_{< ε}
