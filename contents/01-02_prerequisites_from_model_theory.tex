...
for a mathematical object to \emph{model} a \emph{theory}. In this section I will closely follow Chapter 1 of the textbook \cite{Marker2002}.
...
A model on the other hand is describes the \emph{semantics} of an object. It
gives concrete interpretation of the symbols of a language and tells us, how the
formulae are to be interpreted.

\begin{defin}
  A \emph{language} \(\lang\) is a quadruple \((\mathcal{F}, \mathcal{R},
  \mathcal{C}, ar: \mathcal{F} ∪ \mathcal{R} → ℕ)\), where \(\mathcal{F}\) is a
  set of function symbols, \(\mathcal{R}\) is a set of relation symbols, and
  \(\mathcal{C}\) is a set of constant symbols, such that all of these sets are
  pair-wise disjoint. The function \(ar: \mathcal{F} ∪ \mathcal{R} → ℕ\) assigns
  to every function symbol \(f ∈ \mathcal{F}\) and every relation symbol \(R ∈
  \mathcal{R}\) the \emph{arity} \(n_f\) and \(n_R\) respectively.
\end{defin}
...
Analogously, the arity \(n_R\) of an relation symbol \(R\) describes that \(R\)
...
\begin{exam}
  ...
  \begin{exlist}
    ...
    \cdot; 0, 1}}\), where \(\mathtt{+, -}\) and \(\mathtt{\cdot}\) are binary function symbols and \(\mathtt{0, 1}\) are constants.
  \end{exlist}
\end{exam}
...
\begin{defin}
  Let \(\lang = \set{f ∈ \mathcal{F}; R ∈ \mathcal{R}; c ∈ \mathcal{C}}\) be a language. A \emph{model} \(\mathfrak{A}\) of \(\lang\) is a non-empty set \(A\) together with
  \begin{thmlist}
...
\begin{exam}
  ...
  \begin{exlist}
    ...
    \item An example of model in language of (reflexive) orderings \(\lang_{≤} =
    \set{\mathtt{≤}}\) is \(\mathfrak{N}_{≤} := ⟨ℕ, ≤⟩\), where \(≤\) denotes
    the usual ordering of the non-negative integers.
...
\begin{defin}
  Let \(\lang = \set{f ∈ \mathcal{F}; R ∈ \mathcal{R}; c ∈ \mathcal{C}}\) be a language. The set of \emph{\(\lang\)-terms} is the smallest set \(T(\lang)\), such that
  \begin{thmlist}
    \item every constant symbol \(c ∈ \mathcal{C}\) is a term,
    \item every variable symbol \(\mathtt{x_i}\) is a term (\(i ∈ ℕ\)), and
    \item if \(\seq[n_f]{t} ∈ T(\lang)\) are terms then \(f(\seq[n_f]{t})\) is a term for all function symbols \(f ∈ \mathcal{F}\).
  \end{thmlist}
\end{defin}
...
\begin{lem}\label{lem:terms of rings are polynomials}
  ...
  to the set of polynomials with integral coefficients \(ℤ[X_1,
  X_2, …]\).
\end{lem}
\begin{proof}
  ...
  \begin{plist}
    \item If \(t = c\) is a constant, then \(t = 0\) or \(1\). Both are constant
...
Finally, we can define formulae in a language.
...
\begin{defin}
  ...
  \begin{thmlist}[resume]
    ...
    \item If \(ϕ(\mathtt{x}) ∈ Φ(\lang) \) is a formula containing at least the
    variable \(\mathtt{x}\) in one of its terms, then \(\mathtt{∃ x :}
    ϕ(\mathtt{x}) ∈ Φ(\lang)\) is a formula.
  \end{thmlist}
    ...
    \begin{thmlist}[resume]
      ...
      \item If \(ϕ(\mathtt{x}) ∈ Φ(\lang)\) is a formula containing at least the
      variable \(\mathtt{x}\), then we abbreviate \(\mathtt{¬ ∃ x: ¬}
      ϕ(\mathtt{x})\) by \(\mathtt{∀ x:} ϕ(\mathtt{x})\).
    \end{thmlist}
\end{defin}

Note that formulae as defined above are just strings and do not inherit any
meaning or truthfulness. However, once we interpret a formula in a model, we
can say whether the formula is true or false. Let us consider some examples in
the language of rings with one.
...
\begin{defin}
  Let \(\mathtt{x}\) be a variable and let \(ϕ\) be a formula containing
  \(\mathtt{x}\). If \(ϕ\) contains \(\mathtt{∃ x} : ψ(\mathtt{x})\) as a
  sub-formula for some formula \(ψ\), we call this appearance of \(\mathtt{x}\)
  \emph{bound appearance}. All appearances of \(\mathtt{x}\) that are not of
  this shape are called \emph{free appearances}.
\end{defin}

In \cref{ex:formula distributivity} all appearances of \(\mathtt{x_1, x_2}\) and
\(\mathtt{x_3}\) are bound. In (2) variable \(\mathtt{x_2}\) appears bound while
\(\mathtt{x_1}\) is free and in (1) all variables appear free. For a formula
\(ϕ\) we will write \(ϕ(\mathtt{\seq{x}})\) to emphasize that at most the variables \(\mathtt{\seq{x}}\) appear free in \(ϕ\).
...
\begin{defin}
  ...
  \begin{thmlist}
    ...
    \[
      R^{\mathfrak{A}}(t_1^{\mathfrak{A}}(\seq{α}), …, t_{n_R}^{\mathfrak{A}}(\seq{α})).
    \]
    ...
    \item If \(ϕ(\mathtt{\seq{x}}) = \mathtt{∃ x :} ψ(\mathtt{x, \seq{x}})\),
...
\begin{rem}
  \begin{exlist}
    ...
    \item Note that variables can appear both free \emph{and} bound in the same
    formula, e.g. \(\mathtt{x_2}\) in
    \[
    ...
    \emph{either} free \emph{or} bound but not both, and if a variable appears
    bound, then it is bound by a single quantifier. It is easy to check that the
    formula above is true in a model if and only if the following formula is
    true
    \[
      \mathtt{(∃ x_2 : x_1 \cdot x_2 \doteq x_2) ∧ (x_4 + x_3 \doteq x_1)}.
    \]
    Variables that appear free in a formula are also called \emph{free
    variables.}
  \end{exlist}
\end{rem}
...
\begin{defin}
  ...
  universes \(A\) or \(B\) respectively. A function \(φ : A → B\) is called
  ...
  \begin{thmlist}
    ...
    \item \(R^{\mathfrak{A}} (\seq[n_R]{α}) ⇒ R^{\mathfrak{B}}(φ(α_1), …,
    φ(α_{n_r}))\) holds for all \(R ∈ \mathcal{R}\) and all \(\seq[n_R]{α} ∈
    A\); and
...
\begin{rem}
  ...
  \(\mathrm{id}_ℕ\) is not a ring-morphism, but it is clearly a
  \(\lang_{ring}\)-morphism.
...
\begin{defin}
  ...
  \begin{thmlist}
    ...
    \item A class of models \(\mathcal{M}\) in is called \emph{elementary class} if there exists an \(\lang\)-theory \(T\) such that for all models \(\mathfrak{A}\) the following equivalence holds
    \[
...
\(\lang_{ring}\)-structure of \(\mathfrak{R}\).
...
\begin{defin}
  ...
  \begin{thmlist}
    ...
    \]
    of all existentially quantified atomic formulae that are satisfied by
    \(\mathfrak{A}\).
    ...
    \]
    of all existentially quantified conjunctions of atomic formulae that are
...
\begin{exam}
  \begin{exlist}
    ...
    \[
      ϕ := \mathtt{∃ x_1 : ∃ x_2 : ∃ x_3 : (E(x_1, x_2) ∧ E(x_2, x_3) ∧ E(x_3, x_1))}
    \]
...
\begin{defin}
  ...
  obtained by first replacing every variable symbol \(\mathtt{x_i}\) in \(ϕ\) by
  the string
  \[
    \mathtt{x}\overbrace{\mathtt{'…'}}^{n\text{-times}}.
  \]
...
\begin{exam}
To get a feeling for how fast the Gödel numbers grow let us consider the Gödelization of the following \(\lang_{ring}\)-formula
\[
  \mathtt{∃ x_1 : x_1 \doteq 0}.
\]
We choose the function \(i\) as described below.
\[
...
\begin{defin}
  Let \(\lang\) be an at most countable language. An \(\lang\)-theory \(T\) is \emph{decidable} (\emph{semi-decidable}) if the set
  \[
    \set{\enc{ϕ} \mid ϕ ∈ T}
  \]
