...
\begin{lem}\label{lem:terms of rings are polynomials}
  Let \(R\) be a ring with one and \(\mathfrak{R}\) its associated
...
with do not matter. The interested reader may whish to read the excellent
...
the Gödelization. Thus, we can rearrange the symbols of our language. In this
view, a language \(\lang = \set{f ∈ \mathcal{F}; R ∈ \mathcal{R}; c ∈
\mathcal{C}}\) is computable if we can encode its basic symbols in such a way
that
\begin{enumerate}
...
\begin{rem}
  \begin{exlist}
    ...
    the sense of the definition above and \(α_R\) is a
...
The following alternative characterization of computably presentable models can
...
\begin{lem}
  ...
  \begin{exlist}
    \item \(\mathfrak{A}\) is computably presentable as a model in \(\lang\).
    \item \(\mathfrak{A}\) is computably presentable as a model in \(\lang_A\).
...
\begin{exam}
  \begin{exlist}
    ...
    and \(\cdot\) are also easily seen to be computable. As for \(+\) the
    algorithm takes the input
    \[
      \one … \one \zer \one … \one
    \]
    ...
    \(n ≤ m\) if and only if \(\max(n - m, 0) = 0\). So deciding \(n ≤ m\) boils
...
model are computably isomorphic, i.e.\ if there exists a computable
...
In the case of rings of algebraic integers (see~\cref{sec:number theory}) the
following theorem applies and assures us that the decidability of
\textsc{H10} does in fact not depend on the encoding chosen. For a more detailed
discussion of efficient representations of rings see the
textbook~\cite{Stoltenberg1999}.
...
Let \(ζ_1, …, ζ_n ∈ R\) be a set of generators of \(R\) over \(R\) and let
\(φ_1: R → R_1, φ_2: R → R_2\) be the efficient representations of \(R\)
together with the respective ring isomorphisms. Then \(φ_1(ζ_1), …, φ_1(ζ_n)\)
generate \(R_1\) over \(R_1\) and \(φ_2(ζ_1), …, φ_2(ζ_n)\) generate \(R_2\)
over \(R_2\). Storing these finitely many values of the isomorphism \(φ_2 \circ
φ_1^{-1}\) in memory one can use the computabilty of \(R_1\) and \(R_2\)
respectively to extend the partial mapping in the obvious way.
...
statemant in one encoding into a statement in the other encoding and vice versa.
...
\begin{lem}\label{lem:H10 is semi decidable}
  Let \(R\) be a computable, commutative ring with one and \(\mathfrak{R}\) its
  \(\lang_{ring}\)-structure. Then the Diophantine theory
...
\begin{proof}
  ...
  efficiently. Thus, the relation \(H ⊂ R^2\) defined by
  \[
