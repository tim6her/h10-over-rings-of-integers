...
\begin{defin}
  ...
  \begin{thmlist}
    ...
    \(i\) the arity of \(f_i\).
    \item The signature \(Σ\) is \emph{computable} if \(I \subseteq ω\) is
    decidable and \(\mathrm{ar}\) is computable.
  \end{thmlist}
\end{defin}

We identify constant symbols with \(0\)-ary function symbols. This means that in a structure \(\mathfrak A = ⟨A, Σ⟩\) the constant \(\mathtt{c}\) is interpreted as
\[
  \mathtt{c}^{\mathfrak A}: A^0 = \set{∅} → A.
\]
...
% TODO
\todo{§7, Shlapentokh2007: Vertical Methods of Denef and Lipshitz}
