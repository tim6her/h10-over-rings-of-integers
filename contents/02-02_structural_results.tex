...
vertical method} was developed by Denef and Lipshitz and successfully applied
...
\begin{thm}\label{thm:strong vertical method}
  ...
  \begin{align}
    ...
    \text{ for all } 1 ≤ i ≤ n, \\
    \label{eq:sigma alpha and norm of y}& |σ_i(α)| ≤ |N_{L/ℚ}(y)|^c
...
\begin{proof}
  ...
  \eqref{eq:sigma x and norm of y} and \eqref{eq:sigma alpha and norm of y}
  imply that
  \[
    |N_{L/ℚ}(x - α)| = \prod_{i=1}^n |σ_i(x - α)| ≤
    \prod_{i=1}^n 2 \max(|σ_i(x)|, |σ_i(α)|) ≤ 2^n |N_{L/ℚ}(y^{cn})|.
  \]
  From these two inequalities above it follows that
  \[
    |N_{L/ℚ}(y^{2cn})| ≤ |N_{L/ℚ}(y^{cn})|.
  \]
  Now since \(y\) is not a unit, the modulus of its norm is greater than \(1\).
  We conclude that \(|N_{L/ℚ}(y^{cn})| < |N_{L/ℚ}(y^{2cn})|\) yielding the
