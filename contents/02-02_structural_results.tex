...
\begin{exam}
  \begin{exlist}
    ...
    and as a consequence is computable. See \cite{Stoltenberg1999} for a more
    detailed discussion and \cref{app:polynomials} for a sample implementation
    based on this idea.
...
x_n)$. Both operations are computable as $\algint$ is computable. If $p(x_1, …,
x_n) = 0$ the algorithm finishes, otherwise it writes the next string on the
...
\[ S_1 ∪ S_2 = \lbrace \mathbf{α} \mid \mathbf{β} \; h(\mathbf{x}, \mathbf{y}) = 0 \rbrace. \]
