...
\begin{rem}
  A direct consequence of this definition of problems and encodings is that one
...
\begin{exam}\label{ex:add 1}
    ...
    enter the state \(\state{error}\). The complete run of \(\mathbb
    A_\text{add1}\) on \(\one\one\zer\one\)---encoding the number \(11\)---can
    be seen in \cref{fig:complete run}.
\end{exam}
...
\begin{exam}
    ...
    second case it suffices to check that the string has length \(1\) or ends in
    a \(\one\), and in the third case every string is accepted.
\end{exam}
...
\begin{exam}[Pairing functions]
  \begin{exlist}
    ...
    integer, one only needs to check wether \(x\) contains a single \(\zer\)
...
\begin{figure}
  \includegraphics[height=30ex]{res/Pinocchio}
...
\end{figure}
\footnotetext{%
Own \textsc{SVG}-tracing, derived of an illustration by Carlo Chiostri published
in Collodi, Carlo. \begin{italian}\itshape Le avventure di Pinocchio~: Storia di
un burattino.\end{italian} Firenze Bemporad \& Figlio, 1901.
% As Carlo Chiostri died in 1939, this work is in the public domain
% in its country of origin (Italy) and other countries and areas where the
% copyright term is the author's life plus 70 years or less. In particular, it
% is in the public domain in Austria.
}
...
\begin{proof}
    ...
    A}, \enc{\mathbb A}) = \zer\). Expressed more formally this means
    \[
...
reducibility}. One writes \(Q ≤_m Q'\) in this situation. In the previous remark
