...
\begin{proof}
  ...
  induction on \(n\) one sees easily that \(R\) is decdidable as a ternary
  relation. (If \(R(n, x, y)\) holds, so does \(R(m, x, y)\) for all \(m ≥ n\),
  and if \(\mathbb{A}\) did not halt on \(x\) in \(n\) steps, we perform one
  more step of the computation and can check wether \(\mathbb{A}(x) = y\)
