...
will be made precise in ???.\todo{Reference}
...
using the \emph{Haskell} programming language.\todo{Write Appendix}
...
\begin{proof}
  ...
  reach a halting state run $\mathbb B_2$.
\end{proof}
...
\begin{exam}[Pairing functions]
  \begin{exlist}
    \item Using tally notation on can encode $(n, m) ∈ ℕ^2$ by
    \[
...
\todo{Partial recursive functions}
\todo{Church--Turing thesis}

\subsubsection{Halting problem}

%One can extend the alphabet of Turing machines by encoding characters
%i.e.~assigning bit sequences to them. Very common encodings are \textsc{ASCII}
%and \textsc{UTF-8}\footnote{See the Appendix for an \textsc{ASCII} encoding
%table and details on \textsc{UTF-8}.}. Using either one of these encodings the
%string `haskell' is encoded as
%
%\begin{lstlisting}
%h         a         s         k         e         l         l
