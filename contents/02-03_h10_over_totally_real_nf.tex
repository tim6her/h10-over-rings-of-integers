...
\begin{lem}
  ...
  \begin{thmlist}
    ...
    \(\py_m\) divides \(\py_{mk}\) in \(\algint\) and
    ...
    \((\px_m)\) and \(\py_m\) are relative prime in \(\algint\) for all \(m ∈
    ℕ\) i.e.\ \((\px_m) + (\py_m) = \algint\).
  \end{thmlist}
\end{lem}
\begin{proof}
  \begin{plist}
    ...
    \[
      1 = \px_m^2 - (a^2 - 1) \py_m^2 ∈ (\px_m) + (\py_m).
    \]
  \end{plist}
\end{proof}
...
\begin{lem}
  ...
  \begin{thmlist}
    \item \(\py_{eh} | \py_{em}\) implies \(h | m\) in \(ℤ\) and
    \item \(\py_{eh}^2 | \py_{em}\) implies \(h \py_{eh} | m\) in \(\algint\).
  \end{thmlist}
\end{lem}
\begin{proof}
  ...
  Let \(\py_{eh} | \py_{em}\) and  set \(m = t h + r\) for \(t, r ∈ ℕ\) with \(0 ≤ r < h\). Assume to reach a contradiction that \(r > 0\), then
  \[
    \py_{em} = \py_{eth + er} \overset{\text{\cref{lem:addition formulas}}}{=}
  ...
  er}. Consequently, \(r = 0\) and \(h | m\) in \(ℤ\).

  To see the second divisibility condition we assume \(\py_{eh}^2 | \py_{em}\)
  in \(\algint\). Then by the first part of the lemma, we know that there exists
  an integer \(t\) such that \(m = th\). Using the binomial formula we obtain
  \begin{align*}
