...
\begin{proof}
  \begin{plist}
    ...
    found in \cite[§~34, Lemma~(a)]{Hecke1981}. Notice that all roots
...
\eqref{eq:def of x and y} in the special cases we are considering in this
section one can derive further properties.

\begin{lem}
  Let \(K ≠ ℚ\) be a number field and let \(a ∈ \algint\) satisfy
  \eqref{eq:approximations of a}. If \(s\) is the number of pairs of non-real
  embeddings of \(K\) into \(ℂ\) then
  \[
    m < |σ_i(\px_m)|
  \]
