...
\begin{rem}
  ...
  conjugation.
  % TODO
  \todo{Think of some other notation for \(σ_1^{K}\)}
\end{rem}
...
(\cref{thm:Dirichlet}) we find \(\rk U_K = n\). By \cref{lem:r and s for tr and
opnr} we find that \(\rk U_L = 1 + n\). If on the other hand, \(K\) satisfies
...
even more is true, as \(ε\) one can set \(ε = ε_0\), but before we can prove
...
\begin{proof}
  ...
  We assert without loss of generality that \(ε = ζε_0^k\), where \(k\) is a
  positive integer. Then for all \(1 ≤ i ≤ n\) and all \(1 ≤ j ≤ 2\), we have
  \[
    |σ_{ij}(ε)| = |σ_{ij}(ζε_0^k)| = |σ_{ij}(ε_0)|^k.
  \]
  Furthermore, note that \(σ_{i1}(ε_0^{-1}) = σ_{i2}(ε_0)\) for all \(1 ≤ i ≤
  n\).
  ...
  \begin{plist}
    ...
    To see the second inequality, we notice that
    \[
      |σ_{ij_0}(ε_0) - σ_{ij_0}(ε_0^{-1})| =
      |σ_{ij_0}(ε_0)^{-1}| |σ_{ij_0} (ε)^2 - 1| < 2,
    \]
    where \(j_0 ∈ \set{1, 2}\) is chosen in such a way that \(|σ_{ij_0}(ε)| ≤
    1\).
  \end{plist}
\end{proof}
...
\end{lem}
\begin{proof}
  By the discussion above the torsion free part of \(S\) is a free \(ℤ\)-module
  of rank \(1\). Thus there exists an \(ε_0 ∈ S\) and a root of unity \(ζ\) such
  that \(η = ζ ε_0^k\) for some integer \(k\). Furthermore, since the unit
  \(ε_0\) cannot be a root of unity, we may demand \(|ε_0| > 0\), for otherwise
  we replace \(ε_0\) by its inverse.

  We apply this observation to \(ε\) and find \(ε = ζ ε_0^k\), where \(k ≠ 0\).
  I claim that \(|k| = 1\). Then \(ε_0 = ζ^{-k} ε^{k}\) and the lemma is proven.

  Before proving the claim we make another observation. Let \(ε_0 = α + δ β\)
  for some algebraic integers \(α, β ∈ \algint\), then \(2δ | (ε_0 - ε_0^{-1})\)
  in \(algint\). This is because \(ε_0^{-1} = α - δ β\) (cf.~\cref{lem:subgroup
  of ker N L/K}) and
  \[
    ε_0 - ε_0^{-1} = 2δ\, β.
  \]

  From the observation it follows that
  \[
    |N_{L/ℚ}(2δ)| ≤ |N_{L/ℚ}(ε_0 - ε_0^{-1})|.
  \]
  We will derive some approximations for the modulus of \(a\) from this. For
  this purpose we distinguish two cases. If \(K ≠ ℚ\) is totally real, then by
  \cref{lem:approx for delta i 1} and (ii) we find that
  \[
    |N_{L/ℚ}(2δ)| = 2^{2n} \prod_{i = 1}^n |δ_i|^2 > \frac{2^{2n}}{2^{2n - 2}} \frac{|a|^2}{4} = a^2
  \]
  and
