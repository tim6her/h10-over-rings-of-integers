...
\begin{document}
  ...
  \path
    ...
    (4, -0.5) node (ad) {\(D(\mathfrak{A})\)} --
    (2, 0.5) node (h10) {\(\mathtt{H10}(\mathfrak{A})\)} --
    (2, 1.5) node (de) {\(D_{∃+}(\mathfrak{A})\)} --
    (2, 2.5) node (cd) {\(D^c(\mathfrak{A})\)} --
    (3, 1) node (K) {\(\mathcal{K}\)};

  \foreach \f/\t in {cd/cth, cd/de, cth/the, the/pdt, de/the, h10/pdt}
    \path[draw, very thick, <-]
      (\f) edge (\t);

  \foreach \f/\t in {de/h10, h10/K, K/de}
    \path[darw, very thick, <->]
      (\f) edge (\t);
